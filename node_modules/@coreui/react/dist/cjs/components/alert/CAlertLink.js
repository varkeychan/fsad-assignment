'use strict';

var tslib_es6 = require('../../node_modules/tslib/tslib.es6.js');
var React = require('react');
var PropTypes = require('prop-types');
var index = require('../../_virtual/index.js');
var CLink = require('../link/CLink.js');

const CAlertLink = React.forwardRef((_a, ref) => {
    var { children, className } = _a, rest = tslib_es6.__rest(_a, ["children", "className"]);
    return (React.createElement(CLink.CLink, Object.assign({ className: index.default('alert-link', className) }, rest, { ref: ref }), children));
});
CAlertLink.propTypes = {
    children: PropTypes.node,
    className: PropTypes.string,
};
CAlertLink.displayName = 'CAlertLink';

exports.CAlertLink = CAlertLink;
//# sourceMappingURL=CAlertLink.js.map
